...
int          _prevWheelPos;    //< Previous position of the mouse wheel
/**
 * Clean up and exit
 *
...
void init(void)
{
   try
   {
      ...
      _font = new Font(fontFile, 50);

      std::cout << "Texture size on client, before mipmapping: " << (sizeof(float) * 4 * _font->texWidth() * _font->texHeight() / (1024 * 1024)) << " MB" << std::endl;
      ...
      glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_LINEAR); // There is no GL_LINEAR_MIPMAP_LINEAR for the mag filter
...
int update(double time)
{
   try
   {
      ...
      // Build a zoom in/out translation based on the change in scoll wheel position
      glm::mat4 zoom = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.0f, float(glfwGetMouseWheel() * 0.1f)));
      ...
      glm::mat4 mvp        = projection * zoom * view * translate * model; // Remember, matrix multiplication is the other way around
